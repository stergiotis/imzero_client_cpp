...
static void ImGui_ImplSDL3_InitPlatformInterface(SDL_Window* window, SDL_GLContext sdl_gl_context);
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event)
{
    ...
    switch (event->type)
    {
        ...
        case SDL_EVENT_KEY_UP:
        {
            ...
            ImGuiKey key = ImGui_ImplSDL3_KeycodeToImGuiKey(static_cast<int>(event->key.keysym.sym));
            io.AddKeyEvent(key, (event->type == SDL_EVENT_KEY_DOWN));
            io.SetKeyEventNativeData(key, static_cast<int>(event->key.keysym.sym), event->key.keysym.scancode, event->key.keysym.scancode); // To support legacy indexing (<1.87 user code). Legacy backend uses SDLK_*** as indices to IsKeyXXX() functions.
            return true;
        }
...
static bool ImGui_ImplSDL3_Init(SDL_Window* window, SDL_Renderer* renderer, SDL_GLContext sdl_gl_context)
{
    ImGuiIO& io = ImGui::GetIO();
...
bool ImGui_ImplSDL3_InitForOpenGL(SDL_Window* window, SDL_GLContext sdl_gl_context)
{
