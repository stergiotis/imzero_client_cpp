...
BmpBGRA8888Encoder::BmpBGRA8888Encoder(uint32_t width, uint32_t height) {
...
BmpBGRA8888Encoder::~BmpBGRA8888Encoder() {
    if(fHeader != nullptr) {
        free(fHeader);
    }
}

size_t BmpBGRA8888Encoder::bmpTotalSize() const {
    return fRawSizeInclPadding+fHeaderSize;
}

void BmpBGRA8888Encoder::encode(void *out, const uint32_t *rgbaPixelsTLtoBR) const {
    memcpy(out,fHeader,fHeaderSize);
    memcpy(out, rgbaPixelsTLtoBR, fRawSizeInclPadding);
}

bool BmpBGRA8888Encoder::encode(SkWStream *stream, const uint32_t *rgbaPixelsTLtoBR) const {
